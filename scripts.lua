...
function sanityCheckKeysandLog(outputingRecords)
	...
	for k,v in pairs(outputingRecords) do
		if type(k) ~= "string" then
			...
			local psCmd = 'powershell.exe -Command `Write-EventLog -LogName "Application" -Source "Application Error" -EventID 3045 -EntryType Information -Message "Fluentbit script parsed out event with an unexpected key result: '..key ..'\n B64 dataset: \n '.. b64enc(json.encode(outputingRecords)) ..'" -Category 1 -RawData 10,20`'
			print (psCmd)
			--os.execute(psCmd)
		else 
			output[k] = v
